class CourseVideoForm {
     ...
     */
    constructor(courseVideosService) {
    ...
    calculateVideoLength() {
        this.video.duration.length = Number(this.video.duration.minutes) * 60 + Number(this.video.duration.seconds);
    }
    ...
    async submit(form) {
        ...
        if (this.video.id) {
            let { record } = await this.courseVideosService.update(this.video.id, form);
